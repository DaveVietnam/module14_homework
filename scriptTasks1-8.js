...
//Loading images from localStorage on page load (if any)
window.addEventListener("load", () => {
  const savedPhotos = JSON.parse(localStorage.getItem("photos") || "[]"); //checking for any previously saved "photos", if they do exist we parse them(turn into JS object, an arr with images as objects)

  //for every object we create img tags with all properties we need and add them to our html with appendChild()
...
submitBtn.addEventListener("click", async () => {
  try {
    ...
    const data = await response.json(); //turning the response in the arr with JS objects
    console.log(data);
    // Clear existing content and localStorage, then save new images
    resultDiv.innerHTML = "";
    const photoURLs = data.map((photo) => photo.url); //we got the arr with url of every object
    localStorage.setItem("photos", JSON.stringify(photoURLs)); //we converted this arr with urls into json formatted string(that may look like this "["url.png,", "url.png,","url.png,"]") and saved it in the local storage with "photos" name (exactly what we check above)

    // Display images
    //then we looped through each  url in photoURLs array, and created an img for each url there
